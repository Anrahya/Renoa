...
use crate::{
    InspectionContainerConfig, LocalHostError,
    isolated_workspace::{InspectionContainer, checked_output},
};

pub(super) struct Checkout {
    root: PathBuf,
    pub(super) container: Arc<InspectionContainer>,
}

impl Checkout {
    pub(super) async fn remove_abandoned(
        root: &Path,
        config: &InspectionContainerConfig,
        id: uuid::Uuid,
    ) -> io::Result<()> {
        if tokio::fs::try_exists(root).await? {
            InspectionContainer::remove_for(config, id).await?;
    ...
    pub(super) async fn prepare(
        ...
        InspectionContainer::remove_for(config, snapshot.request.id).await?;
        remove_tree(&root).await?;
        ...
        match InspectionContainer::start(config, snapshot.request.id, &root, cancel).await {
...
// The Host uses a private umask. The unprivileged container must still be able
