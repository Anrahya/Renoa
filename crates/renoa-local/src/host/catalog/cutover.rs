...
pub(crate) fn cutover(path: &Path) -> Result<(), HostCatalogError> {
    ...
    migrate(&mut connection, None).map(|_| ())
}

/// Applies the schema cutover and removes every named agent-owned row in the
/// same transaction, so no catalog failure can expose only one half.
pub(crate) fn cutover_and_clear(
    path: &Path,
    agent_owned_tables: &[&str],
) -> Result<BTreeMap<String, u64>, HostCatalogError> {
    let mut connection = open(path, rusqlite::OpenFlags::SQLITE_OPEN_READ_WRITE)?;
    migrate(&mut connection, Some(agent_owned_tables)).map(Option::unwrap_or_default)
}

fn migrate(
    connection: &mut Connection,
    clear_tables: Option<&[&str]>,
) -> Result<Option<BTreeMap<String, u64>>, HostCatalogError> {
    ...
    let removed = clear_tables
        .map(|tables| clear_agent_rows(&transaction, tables))
        .transpose()?;
    verify(&transaction)?;
    #[cfg(test)]
    fail_before_commit_if_armed()?;
    transaction.commit()?;
    Ok(removed)
}
...
#[cfg(test)]
pub(crate) fn fail_next_clear_before_commit() {
    FAIL_BEFORE_COMMIT.set(true);
}

#[cfg(test)]
fn fail_before_commit_if_armed() -> Result<(), HostCatalogError> {
    if FAIL_BEFORE_COMMIT.replace(false) {
        return Err(HostCatalogError::Invalid(
            "injected catalog failure before reset commit".to_owned(),
        ));
    }
    Ok(())
}
