...
use crate::{
    AgentDefinitionError, AgentDocuments,
    atomic_file::content_hash,
    capabilities,
...
fn document_root(
    ...
    let data_directory = fs::canonicalize(data_directory).map_err(|source| {
        document_io("resolve Host data directory", data_directory, source)
    })?;
    let root = data_directory.join(DOCUMENT_DIRECTORY).join(agent.to_string());
    fs::create_dir_all(&root).map_err(|source| document_io("create agent document directory", &root, source))?;
...
fn restrict_directory(path: &Path) -> Result<(), AgentDefinitionError> {
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt as _;
        fs::set_permissions(path, fs::Permissions::from_mode(0o700)).map_err(|source| {
            document_io("restrict agent document directory", path, source)
        })?;
    }
    Ok(())
}
...
fn publish_document(path: &Path, content: &str) -> Result<(), AgentDefinitionError> {
...
fn require_regular_file(path: &Path) -> Result<(), AgentDefinitionError> {
    ...
    fn execute(
        ...
        Box::pin(async move {
            ...
            if cancellation.is_cancelled() {
                return Err(ToolError::cancelled("agent document update was cancelled", false));
            }
