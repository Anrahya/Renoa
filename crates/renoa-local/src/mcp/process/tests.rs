...
fn typed_remote_failure_survives_the_process_boundary() {
    let parsed = parse_discovery_record(
        br#"{"wire_version":8,"event":"failed","failure":{"kind":"incompatible_protocol","certainty":"definite","message":"wrong revision","partial_changes_possible":false,"diagnostic":{"code":"protocol_version_mismatch","http_status":409,"detail":"server omitted the pinned revision"}}}
"#,
    )
...
fn unknown_failure_class_is_not_accepted_as_a_current_wire_record() {
    let error = parse_discovery_record(
        br#"{"wire_version":8,"event":"failed","failure":{"kind":"maybe","certainty":"definite","message":"ambiguous","partial_changes_possible":false,"diagnostic":{"detail":"invalid class"}}}
"#,
    )
...
fn discovery_accepts_exactly_one_terminal_record() {
    let record = br#"{"wire_version":8,"event":"failed","failure":{"kind":"protocol","certainty":"definite","message":"bad","partial_changes_possible":false,"diagnostic":{"detail":"bad"}}}
"#;
