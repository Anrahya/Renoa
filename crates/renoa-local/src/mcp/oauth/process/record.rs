...
"#;
    ...
    fn credential_state_is_bound_to_the_requested_mcp_endpoint() {
        let record = br#"{"wire_version":5,"event":"oauth_authorized","authorization":{"scheme":"bearer","token":"must-not-escape"},"oauth_state":{"schema_version":1,"mcp_endpoint":"https://first.example/mcp","csrf_state":"expected","redirect_uri":"http://127.0.0.1:3210/oauth/callback"}}
"#;
    ...
    fn oauth_failures_cannot_echo_request_secrets_across_the_host_boundary() {
        let record = br#"{"wire_version":5,"event":"oauth_failed","failure":{"kind":"protocol","certainty":"definite","message":"access-one and code-one were rejected","partial_changes_possible":true,"diagnostic":{"code":"access-one","detail":"code-one failed"}},"oauth_state":{"schema_version":1,"mcp_endpoint":"https://mcp.example/mcp","csrf_state":"state-one","redirect_uri":"http://127.0.0.1:3210/oauth/callback","access_token":"access-one"}}
"#;
