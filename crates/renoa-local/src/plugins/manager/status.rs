...
impl PluginManager {
    pub(crate) async fn connection_statuses(
        ...
        Ok(tokio::task::spawn_blocking(move || {
            catalog.profile_connection_statuses(&agent_id.to_string())
        })
        .await??)
    }
    ...
    pub(crate) async fn disconnect_profile(
        ...
        Ok(tokio::task::spawn_blocking(move || {
            catalog.disable_profile_connection(&agent_id.to_string(), &connection_id)
        })
        .await??)
    }
    ...
    pub(crate) async fn enable_profile(
        ...
        let connection_id = connection_id.into();
        Ok(tokio::task::spawn_blocking(move || {
            catalog.enable_profile_connection(&agent_id.to_string(), &connection_id)
        })
        .await??)
    }
}
