...
use crate::{HarnessError, checkpoint::ContextEntry, schema::json_error};
...
fn compact_message(message: &Message) -> Result<Value, HarnessError> {
    match message {
...
fn compact_tool_result(result: &ToolResult) -> Result<Value, HarnessError> {
...
fn sha256_json(value: &impl Serialize) -> Result<String, HarnessError> {
