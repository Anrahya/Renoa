...
async function main(): Promise<void> {
  if (process.stdin.isTTY) {
    throw new Error(
      "OpenCode Go authentication requires an API key piped on standard input; see adapters/model-provider-node/README.md",
    );
  }
  ...
  try {
    credentials.write("opencode-go", { type: "api_key", key: apiKey });
    process.stdout.write("OpenCode Go API key stored.\n");
  } finally {
    credentials.close();
  }
}
