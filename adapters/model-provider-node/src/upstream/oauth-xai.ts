...
import { pollOAuthDeviceCodeFlow, type OAuthDeviceCodePollOptions } from "./device-code.js";
...
function parseDeviceCode(body: JsonObject): XaiDeviceCode {
	...
	if (verificationUriComplete !== undefined) {
		parsed.verificationUriComplete = verificationUriComplete;
	}
	if (intervalSeconds !== undefined) {
		parsed.intervalSeconds = intervalSeconds;
	}
	return parsed;
}
...
async function pollForTokens(device: XaiDeviceCode, signal: AbortSignal): Promise<OAuthCredential> {
	const pollOptions: OAuthDeviceCodePollOptions<OAuthCredential> = {
	...
	if (device.intervalSeconds !== undefined) {
		pollOptions.intervalSeconds = device.intervalSeconds;
	}
	return pollOAuthDeviceCodeFlow(pollOptions);
}
...
async function loginXai(interaction: ProviderAuthInteraction): Promise<OAuthCredential> {
	...
	const deviceEvent: Extract<AuthEvent, { type: "device_code" }> = {
		type: "device_code",
	...
	if (device.intervalSeconds !== undefined) {
		deviceEvent.intervalSeconds = device.intervalSeconds;
	}
	interaction.notify(deviceEvent);
