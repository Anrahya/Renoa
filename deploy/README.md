...
`0700`, and the service umask keeps the SQLite journal owner-only. Tasks and
node enrollment still come from the trusted coordinator administration API.
Create a surface enrollment inside a transient systemd sandbox so it sees the
same protected state directory:
...
This local bootstrap command does not create a remote administration protocol.
