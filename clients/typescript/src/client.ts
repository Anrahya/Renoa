...
import {
  ...
  uuid,
  type RcpErrorCode,
  type RcpSurfaceClientOptions,
  type ServerMessage,
  type TaskEvent,
  type TaskSummary,
} from "./protocol.js";
import { SurfaceState, type PendingTextCommand } from "./state.js";
...
export class RcpSurfaceClient {
  ...
  async attach(taskId: string, apply: ApplyTaskEvent): Promise<void> {
    ...
    const attachment: Attachment = {
      taskId,
      apply,
      cursor: this.#state.cursor(taskId),
      replay: undefined,
    };
  ...
  submitText(taskId: string, text: string): CommandSubmission {
    uuid(taskId, "taskId");
  ...
  async #sendPendingCommand(command: PendingTextCommand): Promise<void> {
    ...
    this.#state.removeCommand(command.commandId);
  }

  #attachExisting(attachment: Attachment): Promise<void> {
    attachment.cursor = this.#state.cursor(attachment.taskId);
    return this.#request(
      (requestId) => ({
        type: "attach",
        request_id: requestId,
  ...
  async #applyTaskEvent(event: TaskEvent): Promise<void> {
    ...
    this.#state.advanceCursor(event.taskId, event.sequence);
