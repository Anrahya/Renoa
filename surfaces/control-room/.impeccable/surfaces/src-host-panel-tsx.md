...
FIRST VIEWPORT: Compact System / Agents / Work / Library navigation, factual heading, searchable Host map, compact status symbols, and schedule countdowns. Work and shared resources have separate destinations. Saved preview data is clearly dated and read-only. A GitHub review icon means repository policy assigns that agent; MCP selection is not a chat surface binding.

FORM: Circular nodes, measured continuous Host connections, and indented target schedules. Search spans every agent. Large inventories scroll without dropping records; mobile becomes an open vertical map. Agent pages retain detailed controls. A connected-feed ring, brief sweeps on changed execution records, and ticking due times use native CSS with pause and reduced-motion support. No short-lived helper nodes, imagined communication, or inferred worker liveness. Direct factual copy replaces metaphoric taglines. This iteration inherits the user-pinned code-led world; no new form roll or comp round.
