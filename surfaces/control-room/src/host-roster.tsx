...
export function AgentEntry({ host, agent, earlier = false }: { host: HostSnapshot; agent: Agent; earlier?: boolean }) {
  ...
  return <a href={agentHref(agent.id)} className={earlier ? "host-earlier-agent" : "host-agent-entry"}>
    <span className="host-agent-name"><span className="host-roster-identity"><AgentAvatar name={agent.name} github={repositories > 0} />{displayName(agent.name)}</span><ArrowRight size={22} aria-hidden="true" /></span>
