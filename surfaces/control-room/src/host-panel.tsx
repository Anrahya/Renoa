...
export function HostPanelView({ host, preview = false }: { host: ReturnType<typeof useHost>; preview?: boolean }) {
  const [view, setView] = useState<View>(preview ? "agents" : "work");
  const [selected, setSelected] = useState<string | null>(preview ? host.snapshot?.agents.find(a => a.name === "X Desk")?.id ?? null : null);
  ...
  return <div className="host-app"><header className="host-header">
    ...
    {signedIn && <nav aria-label="Host navigation">{(["work", "agents", "connections"] as const).map(item =>
      <button key={item} aria-current={view === item ? "page" : undefined} onClick={() => navigate(item)}>
        {item.charAt(0).toUpperCase() + item.slice(1)}</button>)}</nav>}
    ...
    {host.status === "locked" || host.status === "forbidden" ? <HostLogin refresh={host.refresh} forbidden={host.status === "forbidden"} />
      : host.snapshot ? <>
        {view === "work" && <WorkView host={host.snapshot} openAgent={openAgent} />}
        {view === "agents" && <AgentsView host={host.snapshot} selected={selected} openAgent={openAgent} connections={() => navigate("connections")} />}
        {view === "connections" && <ConnectionsView host={host.snapshot} />}
