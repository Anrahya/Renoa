...
import { usePreviewWork } from "./work-state";
...
type Props = { configuration: ConfigurationEditor; page: AgentPage; navigate: (section: AgentSection) => void; execution: string | undefined; agentId: string; agentName: string };
export function AgentWorkPreview({ page, navigate, execution, agentId, agentName, configuration }: Props) {
  const work = usePreviewWork();
  const example = work.exampleFor(agentId);
  const { automations, executions } = example;
  const enabled = Object.fromEntries(automations.map(item => [item.id, item.enabled]));
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      if (pendingFocus.current && page === "automations") {
        ...
        const element = document.getElementById("automation-inspection-title");
  ...
  return <>
    <TabsContent value="configure" forceMount hidden={!!execution || page !== "configure"}><div className="agent-work-preview"><ConfigurePreview {...configuration} /></div></TabsContent>
    <TabsContent value="overview" forceMount hidden={!!execution || page !== "overview"}><div className="agent-work-preview"><WorkOverview {...{ enabled, openRun, openAutomation, navigate, ...example }} /></div></TabsContent>
    <TabsContent value="automations" forceMount hidden={!!execution || page !== "automations"}><div className="agent-work-preview"><AutomationTimeline {...example} enabled={enabled} setEnabled={(id, value) => work.setEnabled(agentId, id, value)} selected={selectedAutomation} select={selectAutomation} openRun={openRun} /></div></TabsContent>
    <TabsContent value="activity" forceMount hidden={!!execution || page !== "activity"}><div className="agent-work-preview"><ActivityExplorer {...{ filter, setFilter, openRun, executions }} /></div></TabsContent>
