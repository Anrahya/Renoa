...
import { PageHeading, agentCount, useDesignAgents, NoResults } from "./shared";
...
export function SystemPreview({ host }: { host: HostSnapshot }) {
  ...
  function restart() { setRestarts(value => value + 1); if (target === "kernel") setCondition("ready"); confirm(false); setNotice(`${target === "kernel" ? "Execution node" : "RCP coordinator"} restarted in this preview. Saved records are unchanged; interrupted runs still need review.`); }
    ...
    <Tabs value={tab} onValueChange={value => { setTab(value); confirm(false); }} className="gap-7"><TabsList variant="line" aria-label="System sections"><TabsTrigger value="health">Health</TabsTrigger><TabsTrigger value="diagnostics">Diagnostics <Badge variant="secondary">{diagnosticRows.length}</Badge></TabsTrigger><TabsTrigger value="recovery">Recovery</TabsTrigger></TabsList>
      <TabsContent value="health">
        <section className="system-architecture" aria-label="System components"><div className="desk-section-heading"><h2>One system, separate responsibilities</h2><span className="desk-note">Select a component to inspect it</span></div>
          ...
          <div className="system-storage"><Database size={17} /><strong>Durable state</strong><span>Task journal at the coordinator</span><span>Execution records at the node</span><Badge variant="outline">{offline ? "Last observed: saved" : "Persisted"}</Badge></div>
        ...
        <div className="system-destinations"><a href="#agents"><strong>{agentCount(agents.length)}</strong><span>Explore the agent map</span><ArrowRight size={16} /></a><a href="#library"><strong>{capabilityPlugins.length} plugins</strong><span>Shared capabilities</span><ArrowRight size={16} /></a><a href="#library/accounts"><strong>{Object.values(connected).filter(Boolean).length} accounts connected</strong><span>Access & credentials</span><ArrowRight size={16} /></a></div>
      </TabsContent>
      ...
      <TabsContent value="recovery"><section className="system-recovery"><div><h2>Restart a component</h2><p>Recover an execution process without removing its durable work.</p><p className="desk-note">A restart doesn’t prove a failed run completed or retry its external side effects. Inspect interrupted work after recovery.</p></div>
        <div className="system-recovery-controls"><FieldGroup><Field><FieldLabel htmlFor="recovery-target">Component</FieldLabel><NativeSelect id="recovery-target" value={target} onChange={event => { setTarget(event.target.value); confirm(false); setNotice(""); }}><NativeSelectOption value="kernel">Execution node</NativeSelectOption><NativeSelectOption value="coordinator">RCP coordinator</NativeSelectOption></NativeSelect><FieldDescription>{target === "kernel" ? "Restarts the execution node. Active model and tool calls may be interrupted." : "Restarts task admission and routing. Browsers reconnect to the retained journal."}</FieldDescription></Field></FieldGroup>
          {offline ? <Alert><AlertTitle>Recovery is unavailable from this connection</AlertTitle><AlertDescription>The Host must be reachable to accept a restart. If the machine is down, use the VPS provider’s console.</AlertDescription></Alert> : confirming ? <Alert><AlertTitle>Simulate an {target === "kernel" ? "execution-node" : "RCP-coordinator"} restart?</AlertTitle><AlertDescription>This changes example health only. No process on the VPS will be restarted.</AlertDescription><div className="mt-4 flex gap-2"><Button ref={recoveryAction} onClick={restart}>Simulate restart</Button><Button variant="ghost" onClick={() => confirm(false)}>Cancel</Button></div></Alert> : <Button ref={recoveryAction} variant="outline" className="w-fit" onClick={() => confirm(true)}><ArrowClockwise data-icon="inline-start" />Restart {target === "kernel" ? "execution node" : "RCP coordinator"}</Button>}
