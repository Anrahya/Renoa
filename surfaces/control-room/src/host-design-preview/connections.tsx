...
function Plugin({ plugin, agents, searching }: { plugin: CapabilityPlugin; agents: DesignAgent[]; searching: boolean }) {
  ...
  return <details className="desk-disclosure" open={searching || undefined}>
    <summary><span className="desk-icon"><Cube /></span><span className="desk-disclosure-title"><strong>{plugin.name}</strong><small>{pluginCapabilities(plugin).length} capabilities · {plugin.groups.map(group => group.via ?? group.kind).join(" + ")}</small></span><span className="library-member-count">{members.length} agents</span><span className="desk-status" data-tone={ready ? "good" : "warning"}>{ready ? "Available" : "Needs account"}</span><CaretRight size={17} /></summary>
